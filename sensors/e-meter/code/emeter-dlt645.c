...
enum host_commands_e
{
    ...
    HOST_CMD_SET_RADIO_ID                       = 0x40,
    HOST_CMD_GET_MSP_RADIO_ID                   = 0x41,
    HOST_CMD_GET_HSU_RADIO_ID                   = 0x42,
};
...
static void dlt645_process_rx_message(int port, serial_msg_t *rx_msg, int rx_len)
{
    ...
    // *** Set time from GUI
        lstrRTC_Time.year  = rx_msg->uint8[DLT645_MSG_RX_START_BODY + 2]+ 2000;
        lstrRTC_Time.month = rx_msg->uint8[DLT645_MSG_RX_START_BODY + 3];
        lstrRTC_Time.day   = rx_msg->uint8[DLT645_MSG_RX_START_BODY + 4];
        lstrRTC_Time.hour  = rx_msg->uint8[DLT645_MSG_RX_START_BODY + 5];
        lstrRTC_Time.minute = rx_msg->uint8[DLT645_MSG_RX_START_BODY + 6];
        lstrRTC_Time.second = rx_msg->uint8[DLT645_MSG_RX_START_BODY + 7];

        RTC_CE_Set_Time(&lstrRTC_Time);
...
// **** Send timeinformation to GUI
        lstrRTC_Time = *RTC_CE_Get_Time();
        tx8[DLT645_MSG_TX_START_BODY + 2] = (uint8_t)(lstrRTC_Time.year  -2000);
        tx8[DLT645_MSG_TX_START_BODY + 3] = lstrRTC_Time.month;
        tx8[DLT645_MSG_TX_START_BODY + 4] = lstrRTC_Time.day;
        tx8[DLT645_MSG_TX_START_BODY + 5] = lstrRTC_Time.hour;
        tx8[DLT645_MSG_TX_START_BODY + 6] = lstrRTC_Time.minute;
        tx8[DLT645_MSG_TX_START_BODY + 7] = lstrRTC_Time.second;
    ...
    #endif
    #if defined(TEMPERATURE_SUPPORT)
        tx16[DLT645_MSG_TX_START_BODY_W + 4] = temperature;
    #endif
...
//            if ( next_flash_loc >= (uint16_t *) 0x1900)
//               _NOP();
    ...
    case HOST_CMD_SET_RADIO_ID:

        flash_clr( (int*) gbRadio_ID );               // Clear Flash-Segment B (0x1900-0x197F)
        flash_write_int8( (int8_t *) &gbRadio_ID[0], rx_msg->uint8[DLT645_MSG_RX_START_BODY + 2] );
        flash_write_int8( (int8_t *) &gbRadio_ID[1], rx_msg->uint8[DLT645_MSG_RX_START_BODY + 3] );
        flash_write_int8( (int8_t *) &gbRadio_ID[2], rx_msg->uint8[DLT645_MSG_RX_START_BODY + 4] );
        flash_write_int8( (int8_t *) &gbRadio_ID[3], rx_msg->uint8[DLT645_MSG_RX_START_BODY + 5] );
        flash_secure();
        __enable_interrupt();                         // wir sind im seriellen Interrupt -> damit Radio_SetID funktionieren kann IRQ freigeben
        //tx8[DLT645_MSG_TX_START_BODY]     = rx_msg->uint8[DLT645_MSG_RX_START_BODY];
        //tx8[DLT645_MSG_TX_START_BODY + 1] = Radio_SetID( (uint8_t *) gbRadio_ID ) | 0x80;

        prepare_tx_message(port, 2);
        break;
    case HOST_CMD_GET_MSP_RADIO_ID:
        tx8[DLT645_MSG_TX_START_BODY]     = rx_msg->uint8[DLT645_MSG_RX_START_BODY];
        tx8[DLT645_MSG_TX_START_BODY + 1] = rx_msg->uint8[DLT645_MSG_RX_START_BODY + 1] | 0x80;

        tx8[DLT645_MSG_TX_START_BODY + 2] = gbRadio_ID[0];
        tx8[DLT645_MSG_TX_START_BODY + 3] = gbRadio_ID[1];
        tx8[DLT645_MSG_TX_START_BODY + 4] = gbRadio_ID[2];
        tx8[DLT645_MSG_TX_START_BODY + 5] = gbRadio_ID[3];
        prepare_tx_message(port, 6);
        break;

    case HOST_CMD_GET_HSU_RADIO_ID:
        __enable_interrupt();                         // wir sind im seriellen Interrupt -> damit Radio_GetID funktionieren kann IRQ freigeben
        //tx8[DLT645_MSG_TX_START_BODY]     = rx_msg->uint8[DLT645_MSG_RX_START_BODY];
        //tx8[DLT645_MSG_TX_START_BODY + 1] = Radio_GetID( &tx8[DLT645_MSG_TX_START_BODY + 2] ) | 0x80;
        prepare_tx_message(port, 6);
        break;
