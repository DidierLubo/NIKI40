...
void UART_Put_ByteArray(ST_UART_t* st, BYTE len, BYTE* txData)
{
   if (st)
   {
     uint8_t i;
...
inline BYTE UART_Get_RXD_Anzahl(ST_UART_t* st)
{
   if (st)
...
void UART_Get_ByteArray(ST_UART_t* st, register BYTE *bZielBuffer, register BYTE bMaxBytes)
{
   if (st)
   {
     uint8_t i;

     for(i = bMaxBytes; i > 0; i--)
     {
      *bZielBuffer++ = UART_Get_Byte(st);
     }
   }
}
...
void UART_SetStandardOut(ST_UART_t* st)
{
   stSTDIO = st;
}
